      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-6 pointer-events-none">
        <div
          className={`pointer-events-auto transition-all duration-500 ease-out ${
            scrolled
              ? "bg-[rgba(5,13,26,0.90)] backdrop-blur-2xl rounded-full border border-white/[0.1] shadow-[0_8px_40px_rgba(0,0,0,0.5)] px-5 py-2.5 flex items-center gap-2"
        ...
        >
            ...
            {navItems.map((item) => (
              <button key={item.href} onClick={() => handleNav(item.href)}
                className={`text-sm font-medium px-3 py-1.5 rounded-full transition-all duration-200 ${
                  scrolled ? "text-white/70 hover:text-white hover:bg-white/[0.08]" : "text-white/60 hover:text-white hover:bg-white/[0.06]"
                }`}>
                {item.label}
              </button>
            ))}
          ...
          <button className={`lg:hidden ml-auto p-2 flex flex-col gap-1.5 ${scrolled ? "ml-3" : ""}`}
            onClick={() => setMenuOpen(!menuOpen)} aria-label="Menü öffnen">
